package com.yuqun.main.component.refresh;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator
                            .getResistance()));
            ...
            mDurationToCloseHeader = arr.getInt(R.styleable
							.PtrFrameLayout_ptr_duration_to_close_header,
                    mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable
					.PtrFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable
							.PtrFrameLayout_ptr_keep_header_when_refresh,
    ...
    protected void onFinishInflate() {
        ...
        if (childCount > 2) {
            ...
            errorView.setText(
                    "The content view in PtrFrameLayout is empty. Do you forget to specify its id" +
							" in xml layout file?");
            mContent = errorView;
            addView(mContent);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            PtrCLog.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s",
					getMeasuredHeight(),
                    getMeasuredWidth(), getPaddingLeft(), getPaddingRight(), getPaddingTop(),
					getPaddingBottom());
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                PtrCLog.d(LOG_TAG, "onMeasure content, width: %s, height: %s, margin: %s %s %s " +
								"%s", getMeasuredWidth(),
                        getMeasuredHeight(), lp.leftMargin, lp.topMargin, lp.rightMargin, lp
								.bottomMargin);
    ...
    private void measureContentView(View child, int parentWidthMeasureSpec, int
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mDisableWhenHorizontalMove && !mPreventForHorizontal
                        && (Math.abs(offsetX) > mPagingTouchSlop && Math.abs(offsetX) > Math.abs
						(offsetY))) {
                ...
                if (DEBUG) {
                    boolean canMoveDown = mPtrHandler != null && mPtrHandler.checkCanDoRefresh
							(this, mContent, mHeaderView);
                    PtrCLog.v(LOG_TAG,
                            "ACTION_MOVE: offsetY:%s, currentPos: %s, moveUp: %s, canMoveUp: %s, " +
									"moveDown: %s: canMoveDown: %s",
                            offsetY, mPtrIndicator.getCurrentPosY(), moveUp, canMoveUp, moveDown,
							canMoveDown);
                }

                // disable move when header not reach top
                if (moveDown && mPtrHandler != null && !mPtrHandler.checkCanDoRefresh(this,
    ...
    private void updatePos(int change) {
        ...
        if ((mPtrIndicator.hasJustLeftStartPosition() && mStatus == PTR_STATUS_INIT)
                || (mPtrIndicator.goDownCrossFinishPosition() && mStatus == PTR_STATUS_COMPLETE
        ...
        if (mStatus == PTR_STATUS_PREPARE) {
            ...
            if (isUnderTouch && !isAutoRefresh() && mPullToRefresh && mPtrIndicator
					.crossRefreshLineFromTopToBottom()) {
                tryToPerformRefresh();
            }
            // reach header height while auto refresh
            if (performAutoRefreshButLater() && mPtrIndicator
        ...
        if (DEBUG) {
            PtrCLog.v(LOG_TAG, "updatePos: change: %s, current: %s last: %s, top: %s, " +
							"headerHeight: %s", change,
    ...
    private boolean tryToNotifyReset() {
        if ((mStatus == PTR_STATUS_COMPLETE || mStatus == PTR_STATUS_PREPARE) && mPtrIndicator
				.isInStartPosition()) {
            if (mPtrUIHandlerHolder.hasHandler()) {
    ...
    private void performRefreshComplete() {
        ...
        if (mScrollChecker.mIsRunning && isAutoRefresh()) {
            // do nothing
            if (DEBUG) {
                PtrCLog.d(LOG_TAG, "performRefreshComplete do nothing, scrolling: %s, auto " +
								"refresh: %s",
    ...
    private void sendCancelEvent() {
        ...
        MotionEvent e = MotionEvent.obtain(last.getDownTime(),
                last.getEventTime() + ViewConfiguration.getLongPressTimeout(), MotionEvent.ACTION_CANCEL, last.getX(),
