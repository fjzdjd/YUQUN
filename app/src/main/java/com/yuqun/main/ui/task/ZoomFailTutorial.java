...
public class ZoomFailTutorial {
	...
	public ZoomFailTutorial(View containerView, View expandedView) {
...
//		int firstPosition = ((AdapterView<?>)mThumbViewParent).getFirstVisiblePosition();
		...
		mContainView.findViewById(R.id.task_fail_layout).getGlobalVisibleRect(finalBounds, globalOffset);
		startBounds.offset(-globalOffset.x, -globalOffset.y);
		finalBounds.offset(-globalOffset.x, -globalOffset.y);
		//设置比率
		set_Center_crop(finalBounds);
	...
	public interface OnZoomListener {
		 void onExpanded();//点击后展示大图成功后调用
		 void onThumbed();//点击后缩小回小图时调用
	}
